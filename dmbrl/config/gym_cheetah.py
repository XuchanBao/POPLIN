...
class HalfCheetahConfigModule:
    """
        @brief: migrate the gym module from the mbbl repo
            'gym_cheetah': {
                'path': 'mbbl.env.gym_env.walker',
                'ob_size': 17, 'action_size': 6, 'max_length': 1000
            }
    """
    ENV_NAME = "MBRLGYM_HalfCheetah-v0"
    TASK_HORIZON = 1000
    NTRAIN_ITERS = 300
    NROLLOUTS_PER_ITER = 1
    PLAN_HOR = 30
    INIT_VAR = 0.25
    MODEL_IN, MODEL_OUT = 23, 17  # obs - > 17, action 6
    GP_NINDUCING_POINTS = 300

    def __init__(self):
        # self.ENV = gym.make(self.ENV_NAME)
        from mbbl.env.gym_env import walker
        self.ENV = walker.env(env_name='gym_cheetah', rand_seed=1234,
